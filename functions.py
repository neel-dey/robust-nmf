...
TODO:
    1. Implement fp32 computation instead of just fp64.
...
def initialize_rnmf(data, rank, alg, beta=2, sum_to_one=0, user_prov=None):
    ...
    # Utilities:
    # Defining epsilon to protect against division by zero based on data type:
    '''
    if data.dtype == 'float64':
        # global eps
        eps = 2.3e-16  # slightly higher than actual eps in fp64
    else:
        # global eps
        eps = 1.3e-7  # slightly higher than actual eps in fp32
    '''
    eps = 2.3e-16
...
def update_outlier(data, data_approx, outlier, beta, reg_val):
    ...
    # Horrendously ugly expression below because sklearn's normalize won't deal
    # division by zero explicitly as far as I know and the update is long.

    # return outlier * ((data*bet2(data_approx)) / (bet1(data_approx) +
    #                   reg_val*normalize(outlier+(eps/outlier.shape[0]),
    #                                     norm='l2', axis=0)))
